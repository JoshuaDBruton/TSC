...
def run():
    ...
    train_loader = DataLoader(data, shuffle=True, num_workers=hp.num_workers, batch_size=8)
    val_loader = DataLoader(val_data, shuffle=False, num_workers=hp.num_workers, batch_size=8)
    # test_loader = DataLoader(test_data, shuffle=False, num_workers=hp.num_workers, batch_size=1)

    logger = WandbLogger(name="coco-3dia-20",)
    ...
    model = BNet(4, in_channels=5, use_onehot=False, start_filts=16, depth=8, dilated=True)
